  ...
  <div>
    ...
    <div v-if="selectedBroker">
      ...
      <select v-model="selectedAccountType">
        <option v-for="accountType in selectedBrokerAccountTypes" :key="accountType._id" :value="accountType.type">
          {{ accountType.type }}
        </option>
      </select>
    </div>
    ...
    <button @click="addAccount" :disabled="!selectedBroker || !selectedAccountType || !accountNumber || !balance">
      Add Account
    </button>
  </div>
</template>
